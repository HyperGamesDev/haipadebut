...
public class BGScroller2 : MonoBehaviour{
    ...
    void Start(){
        ...
        if(dir==dir.left||dir==dir.right){//Do the same for X
            ...
            go1.transform.position=new Vector2(go1.transform.position.x+(length+offset)*-dirM,go1.transform.position.y);startpos[1]=go1.transform.position.x;}//Spawn reverse to where they'll go
        else{go1.transform.position=new Vector2(go1.transform.position.x,go1.transform.position.y+(length+offset)*-dirM);startpos[1]=go1.transform.position.y;}
    }
    void FixedUpdate(){
        for(var i=0;i<transform.childCount;i++){
            ...
            if(dir==dir.up||dir==dir.down){
                ...
                if(pos>startpos[i]+length+offset||pos<startpos[i]-length-offset)transform.GetChild(i).position=new Vector2(transform.GetChild(i).position.x,startpos[i]);//Bring back to startpos
            }
                ...
                if(pos>startpos[i]+length+offset||pos<startpos[i]-length-offset)transform.GetChild(i).position=new Vector2(startpos[i],transform.GetChild(i).position.y);
            }
        }
    }
}
